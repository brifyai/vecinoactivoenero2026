    ...
    try {
      ...
      const { data: photoComments } = await supabase
        ...
        .select(`
          ...
          comment,
          created_at,
          user:users!photo_comments_user_id_fkey(id, username, name, avatar),
      ...
      const { data: postReactions } = await supabase
        ...
        .select(`
          ...
          reaction_type,
          created_at,
          user:users!post_reactions_user_id_fkey(id, username, name, avatar),
      ...
      const { data: posts } = await supabase
        ...
        .select(`
          ...
          content,
          created_at,
          author:users!posts_author_id_fkey(id, username, name, avatar)
        `)
        .order('created_at', { ascending: false })
        .limit(5);
