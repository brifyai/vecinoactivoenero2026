-- =====================================================
-- SOLO INSERTAR DATOS - Vecino Activo
-- Sin Realtime, sin policies, solo datos
-- Tiempo estimado: 15 segundos
-- =====================================================

-- Crear tabla photo_comments si no existe
...
-- Posts adicionales (10 nuevos)
INSERT INTO posts (author_id, content, privacy, category, created_at)
SELECT 
  id,
  CASE ROW_NUMBER() OVER ()
    WHEN 1 THEN '¡Hola vecinos! 👋 Me acabo de unir a la comunidad. Vivo en ' || COALESCE(neighborhood_name, 'el barrio') || ' y me encantaría conocer a más personas.'
...
WHERE u.email LIKE '%@vecinoactivo.cl' AND p.author_id != u.id
LIMIT 30
ON CONFLICT DO NOTHING;
...
FROM users WHERE email LIKE '%@vecinoactivo.cl' LIMIT 8
ON CONFLICT (slug) DO NOTHING;

-- Asistentes a eventos (20)
...
WHERE u.email LIKE '%@vecinoactivo.cl' LIMIT 20
...
FROM users WHERE email LIKE '%@vecinoactivo.cl' LIMIT 6
ON CONFLICT (slug) DO NOTHING;

-- Miembros de grupos (25)
INSERT INTO group_members (group_id, user_id, role, joined_at)
SELECT 
  g.id, u.id,
  CASE WHEN g.created_by = u.id THEN 'admin' ELSE 'member' END,
  NOW() - (RANDOM() * INTERVAL '10 days')
FROM groups g CROSS JOIN users u
WHERE u.email LIKE '%@vecinoactivo.cl' LIMIT 25
ON CONFLICT (group_id, user_id) DO NOTHING;

-- Amistades (20)
INSERT INTO friends (user_id, friend_id, status, created_at, accepted_at)
SELECT 
  u1.id, u2.id, 'accepted',
...
FROM users WHERE email LIKE '%@vecinoactivo.cl' LIMIT 4
ON CONFLICT (slug) DO NOTHING;
...
SELECT 
  ...
  END,
  'active', id, (RANDOM() * 30)::INT,
  CURRENT_DATE + INTERVAL '7 days',
  NOW() - (ROW_NUMBER() OVER () * INTERVAL '2 days')
FROM users WHERE email LIKE '%@vecinoactivo.cl' LIMIT 3
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  END,
  (RANDOM() * 10)::INT, NOW()
FROM polls p CROSS JOIN generate_series(1, 4) AS s
LIMIT 12
ON CONFLICT DO NOTHING;

-- Actualizar contadores
UPDATE posts p SET comments_count = (SELECT COUNT(*) FROM comments c WHERE c.post_id = p.id);
UPDATE posts p SET likes = (SELECT COUNT(*) FROM post_reactions pr WHERE pr.post_id = p.id);

-- Resumen
SELECT 'RESUMEN DE DATOS CREADOS' as titulo, '' as detalle
UNION ALL SELECT '  Posts:', COUNT(*)::TEXT FROM posts
UNION ALL SELECT '  Comentarios:', COUNT(*)::TEXT FROM comments
UNION ALL SELECT '  Reacciones:', COUNT(*)::TEXT FROM post_reactions
UNION ALL SELECT '  Eventos:', COUNT(*)::TEXT FROM events
UNION ALL SELECT '  Grupos:', COUNT(*)::TEXT FROM groups
UNION ALL SELECT '  Amistades:', COUNT(*)::TEXT FROM friends
UNION ALL SELECT '  Conversaciones:', COUNT(*)::TEXT FROM conversations
UNION ALL SELECT '  Mensajes:', COUNT(*)::TEXT FROM messages
UNION ALL SELECT '  Proyectos:', COUNT(*)::TEXT FROM projects
UNION ALL SELECT '  Encuestas:', COUNT(*)::TEXT FROM polls;
